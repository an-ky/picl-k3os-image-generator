...
- In the config/ directory, create one configuration file for each device, named as `{MAC}.yaml` (e.g. `dc:a6:32:aa:bb:cc.yaml`). The appropriate file will be used as a config.yaml eventually. Note: Microsoft Windows does not support colon in file names, so when creating configuration files in Microsoft Windows, use dash-separated file names like `aa-bb-cc-dd-ee-ff.yaml`). 
